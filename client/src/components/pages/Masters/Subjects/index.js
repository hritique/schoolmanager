...
const Subjects = ({ setAlert, ...props }) => {
	...
	const onClassSelectHandler = (classId) => {
		if (classId !== '') {
			const selectedClass = classList.filter(
				(grade) => grade._id === classId
			)[0];
	...
	const onEditClickHandler = async (id) => {
		...
		setValues({
			...
			priority: selectedSubject.priority,
		});
	};
				...
				<TabContent>
					<TabContent_Main>
						<Menu
							...
							selected={
								utils.selectedClass === '' ? '' : utils.selectedClass._id
							}
						>
							Class
						</Menu>
							...
							<SideListContainer>
								{utils.selectedClass.subjects.map((subject) => (
									<ListItem
										key={subject._id}
										onDelete={() =>
											onDeleteHandler(utils.selectedClass._id, subject._id)
										}
