...
const Classes = ({ setAlert, ...props }) => {
  ...
  const onCreateHandler = async () => {
    ...
    let newClass;
    try {
      newClass = await functions.addNewClass(values);
    } catch (error) {
  ...
  const onUpdateHandler = async () => {
    let result;
    try {
      result = await axios.patch(`/api/masters/class/${utils.selected}`, {
        ...values,
      });
      result = result.data;
    } catch (error) {
  ...
  const onDeleteHandler = async (id) => {
    ...
    let result;
    try {
      result = await axios.delete('/api/masters/class', {
        data: { deleteId: id },
      });
      result = result.data;
    } catch (error) {
  ...
  const fetchData = async () => {
    const classList = await functions.getClassList();
